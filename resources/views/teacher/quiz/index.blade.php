...
<body class="bg-slate-100 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-100 antialiased min-h-screen transition-colors duration-200">
    ...
    <div class="max-w-[1600px] mx-auto flex p-2 sm:p-3 md:p-4 gap-3 md:gap-4">
        ...
        <main class="flex-1 w-full space-y-4 md:space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5">
                ...
                @forelse ($quizzes as $quiz)
                    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200/60 dark:border-slate-700/60 relative overflow-hidden flex flex-col justify-center items-center text-center min-h-[220px] transition-colors duration-200 group">
                        ...
                        <div class="absolute top-4 right-4 flex gap-1.5 z-10">
                            ...
                            <a href="{{ url('/teacher/tests/' . $quiz->id . '/result') }}" class="w-7 h-7 bg-teal-500 text-white rounded-full flex items-center justify-center hover:opacity-80 transition-opacity shadow-sm" title="Results">
                            ...
                            <button type="button" class="w-7 h-7 bg-rose-500 text-white rounded-full flex items-center justify-center hover:opacity-80 transition-opacity shadow-sm" title="Delete Quiz" onclick="openDeleteModal('{{ $quiz->id }}')">
                        ...
                        <div class="z-10 mt-2">
                            <h3 class="text-xl font-black text-slate-800 dark:text-white uppercase mb-1 tracking-wider">{{ $quiz->title }}</h3>
                            <p class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-4">{{ $quiz->teacher_name ?? 'ADMIN USER' }}</p>
                            ...
                            <div class="flex items-center justify-center gap-3 text-sm font-semibold text-slate-700 dark:text-slate-300">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span>{{ $quiz->question_count }} sorag</span>
                                </div>
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span>{{ floor($quiz->duration / 60) }} min</span>
                                </div>
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-span-full text-center py-12 text-slate-500 dark:text-slate-400 font-semibold">
                        Hiç hili synag tapylmady.
                    </div>
                @endforelse
    ...
    <div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm transition-opacity">
        <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md mx-4 shadow-xl border border-slate-200 dark:border-slate-700 transform transition-all">
            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
                ...
                <button type="button" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors" onclick="closeDeleteModal()">
            ...
            <div class="p-4 border-t border-slate-200 dark:border-slate-700 flex justify-center gap-4">
                <button type="button" class="px-5 py-2.5 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors uppercase text-sm" onclick="closeDeleteModal()">
                    Bes etmek
                </button>
                <form id="deleteQuizForm" action="" method="POST" class="m-0">
                    @csrf
                    @method('DELETE')
    ...
    <script>
        function openDeleteModal(quizId) {
            ...
            form.action = '/teacher/tests/' + quizId;
